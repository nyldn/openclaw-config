...
show_welcome() {
    ...
    echo "╔════════════════════════════════════════════════════════════╗"
    echo "║                                                            ║"
    echo "║   🚀 OpenClaw Post-Install Setup Wizard                   ║"
    echo "║                                                            ║"
    echo "║   This wizard will help you configure:                     ║"
    echo "║   • AI/LLM API keys (Claude, OpenAI, Gemini)              ║"
    echo "║   • CLI tool authentication                                ║"
...
show_summary() {
    ...
    [[ -n "$anthropic_key" && "$anthropic_key" != *"your-key"* ]] && echo "  ✓ Anthropic (Claude)" || echo "  ○ Anthropic (not configured)"
    [[ -n "$openai_key" && "$openai_key" != *"your-key"* ]] && echo "  ✓ OpenAI" || echo "  ○ OpenAI (not configured)"
    [[ -n "$google_key" && "$google_key" != *"your-key"* ]] && echo "  ✓ Google Gemini" || echo "  ○ Google Gemini (not configured)"
    [[ -n "$github_pat" ]] && echo "  ✓ GitHub" || echo "  ○ GitHub (not configured)"
    [[ -n "$todoist_token" ]] && echo "  ✓ Todoist" || echo "  ○ Todoist (not configured)"
    [[ -n "$slack_token" ]] && echo "  ✓ Slack" || echo "  ○ Slack (not configured)"
    [[ -f "$CONFIG_DIR/google-credentials.json" ]] && echo "  ✓ Google Calendar/Drive" || echo "  ○ Google Calendar/Drive (not configured)"
