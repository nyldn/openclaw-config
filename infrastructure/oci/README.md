...
Terraform configuration to deploy openclaw-config on an Oracle Cloud Infrastructure (OCI) ARM instance using cloud-init.

**Target instance:** VM.Standard.A1.Flex (4 OCPUs / 24 GB RAM — free tier eligible)

## Supported Operating Systems

| OS | Variable | Image Source | Import Required |
|---|---|---|---|
| **Ubuntu 24.04 Minimal** (default) | `os_image = "ubuntu"` | OCI catalog | No |
| Debian 12 | `os_image = "debian"` | Custom import | Yes |

Ubuntu 24.04 Minimal ARM is recommended — it's available directly from the OCI platform image catalog, so no manual image import is needed.
...
That's it for Ubuntu (the default). For Debian, see [Debian 12 Setup](#debian-12-setup) below.
...
If you prefer Debian over Ubuntu, you must import the image manually since OCI does not offer Debian as a platform image.
...
5. Copy the resulting image OCID and set in `terraform.tfvars`:

```hcl
os_image          = "debian"
debian_image_ocid = "ocid1.image.oc1.iad..your-debian-image-ocid"
```
